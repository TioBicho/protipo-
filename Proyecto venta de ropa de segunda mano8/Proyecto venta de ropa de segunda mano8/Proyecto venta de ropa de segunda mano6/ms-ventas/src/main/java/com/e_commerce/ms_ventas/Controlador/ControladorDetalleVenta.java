...
public class ControladorDetalleVenta {
    ...
    @GetMapping("/todas")
    public ResponseEntity<CollectionModel<ModeloDetalleVenta>> listar() {
        log.info("Petición GET recibida para listar el historial de detalles de ventas");

        List<ModeloDetalleVenta> detalles = service.obtenerTodos();

        CollectionModel<ModeloDetalleVenta> modelo = CollectionModel.of(detalles);

        modelo.add(linkTo(methodOn(ControladorDetalleVenta.class).listar()).withSelfRel());
